!=============================================================================
!
! $Id: wetdep_update.F90,v 1.23 2013-09-16 19:59:22 jkouatch Exp $
!
! CODE DEVELOPER
!   Dan Bergmann, LLNL
!   dbergmann@llnl.gov
...
!-----------------------------------------------------------------------------
     ...
     &   kel, press3c, press3e, concentration, wet_depos, &
#ifdef MICRO_AEROSOL
     &   humidity, REL_SCAV_EFF_new, &
     &   pr_diag, loc_proc, i1, i2, ju1, j2, k1, k2, &
     &   ilo, ihi, julo, jhi, ilong, ivert, ju1_gl, j2_gl, num_species)
#else
     &   pr_diag, loc_proc, i1, i2, ju1, j2, k1, k2, &
     &   ilo, ihi, julo, jhi, ilong, ivert, ju1_gl, j2_gl, num_species)
#endif
...
!     ----------------------
      ...
      real*8            , intent(in   ) :: rain_zm    (i1:i2,   ju1:j2,   k1:k2)
      real*8            , intent(in   ) :: rain_hk    (i1:i2,   ju1:j2,   k1:k2)
...
!     --------------------------------------------------------------------
!     Calculate fraction of moistq which is associated with convection.
!     If there is no precipitation at the surface, set the fraction
!     equal to the cosine of the latitude.  This is done because
!     occasionally the data has negative values of moistq (condensation),
!     but has no precipitation.  Also, there are some places where convec-
!     tive precipitation is slightly larger than the total precipitation??
!     --------------------------------------------------------------------

      where ((tot_precip(:,:) > 1.0d-5) .and.  &
     &       (con_precip(:,:) > 1.0d-5))

        conv_frac(:,:) = con_precip(:,:) / tot_precip(:,:)

      elsewhere

        conv_frac(:,:) = Spread (coscen(ju1:j2), 1, ilong)
...
!
      l_plus_w_con = 2.0d-6
      l_plus_w_str = 0.5d-6
! sensitivity studies (hyl, 10/10/05)
!      l_plus_w_str = 1.5d-6   !hyl
...
!... reset convective fraction of rain for GEOS4 & GEOS5 at each level
        if ((metdata_name_org (1:4) == 'GMAO' ) .and.  &
     &     (metdata_name_model(1:5) == 'GEOS4' .or.    &
     &      metdata_name_model(1:5) == 'GEOS5')) then
!... add convective rain
          conv_frac(:,:) = rain_zm(:,:,ik) + rain_hk(:,:,ik)
!... divide by total rain
          !mk Feb. 13, 2006
         !where(conv_frac(:,:) > 0.0d0)  &
...
!! sensitivity - supress rainout at temperatures below 258K
!! because of the absence of riming, an assumption used by Koch et al. [1996],
!! Chin et al. [1996], and Liu et al. [2001] (hyl, 10/04/06)
!!        rainout(:,:) = (moistq(:,:,ik) < -1.0d-10)                    !orig
!        rainout(:,:) = (moistq(:,:,ik) < -1.0d-10  .and. &
!     &                  (kel (i1:i2,ju1:j2,ik) >= 258.0d0) )
...
!! Now take into account both partial and total reevaporation of precipitation.
!! hyl, 12/28/05
!
!! partial reevaporation of precipitation: half of the scavenged tracer load is
!! released in the gridbox.
!        where ( washout(:,:) .and. (precip_bot(:,:) >= 1.0d-20) )
!          frac_not_evap(:,:) =  &
!     &      1.0d0 -   &
!     &      ((precip_top(:,:) - precip_bot(:,:)) / precip_top(:,:)) *  &
!     &      0.5d0
!        end where
!
!! total reevaporation of precipitation: all of the scavenged tracer load is
!! released in the gridbox.
!        where ( washout(:,:) .and. (precip_bot(:,:) < 1.0d-20) )
!          frac_not_evap(:,:) = 0.0d0
!        end where
...
!! complete evaporation means the bottom of precipitating column, so
!! reinitialize the area fraction of the grid cell which experiences preciptation.
!! hyl, 12/28/05
!
!!!      where ( (q(:,:) <= 0.0d0) .and. (precip_bot(:,:) < 1d-30) )
!        where ( precip_bot(:,:) < 1.0d-20 )
!          frac_area_con(:,:) = 1.0d-20
!          frac_area_str(:,:) = 1.0d-20
!        end where
