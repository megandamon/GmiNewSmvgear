...
!=============================================================================
!
! $Id: drydep_update.F90,v 1.21 2013-09-16 19:59:22 jkouatch Exp $
!
! CODE DEVELOPER
!   Dan Bergmann, LLNL
!   dbergmann@llnl.gov
...
!
     ...
     &   s_radius, s_velocity, BoxHeightCenter, BoxHeightEdge, &
     &   ireg, iland, iuse, xlai, press, &
     &   pr_diag, loc_proc, chem_opt, tdt,    &
     &   i1, i2, ju1, j2, k1, k2, ilong, ilo, ihi, julo, jhi, ju1_gl,   &
     &   j2_gl, i1_gl, i2_gl, mw, num_species)
!
      use GmiTimeControl_mod   , only : GetSecondsFromJanuary1
      use GmiArrayBundlePointer_mod, only : t_GmiArrayBundle
!
      use GmiSolar_mod        , only : CalcSolarDeclination
!      use GmiSolar_mod        , only : CalcSolarZenithAngle
!
...
!       -----------------------------------------------------------
!
        cz1       (i1:i2) =  BoxHeightCenter(i1:i2,ij)
...
!       ===========
        call Depvel  &
!       ===========
!     &   (ilong, ij, radswg(:,ij), surf_air_temp(:,ij), csza, oxidize,  &
     &   (ilong, ij, press, radswg(:,ij), surf_air_temp(:,ij), cosSolarZenithAngle(:, ij), oxidize,  &
...
!
      subroutine Depvel  &
     &  (npts, ij, press, radiat, tempk, suncos, f0, hstar, xmw, aerosol,  &
...
!
            rsurfc(k,ldt) =  &
     &        Max (1.0d0, Min (rsurfc(k,ldt), 9999.0d0))
!
!
!         =============
          end do KLOOP1
