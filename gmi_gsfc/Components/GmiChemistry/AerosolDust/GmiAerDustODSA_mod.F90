...
! !INTERFACE:
!
      subroutine Aero_OptDep_SurfArea(gridBoxHeight,concentration, &
...
!BOC
      ...
      if(NSADaer.eq.7) then
        MSDENS(6) = 2200.0    !SS (coarse2)
        MSDENS(7) = 2200.0    !SS (coarse3)
      endif
!   The following are ordered according to the mass densities below
      IND(1) = 22
      IND(2) = 29
         ...
         DO L = k1, k2
            DO J = ju1, j2
               DO I = i1, i2
                  ...
                  !==============================================================
                  IF (concentration(isynoz_num)%pArray3D(I,J,L) < synoz_threshold) THEN
...
!                  END IF
     ...
     &                odAer(:,:,:,IRHN ) * qaa_b(2,IND(N)) /qaa_b(4,IND(N))
            END WHERE
         ENDDO
         ...
         !------------------------------------
         WHERE (concentration(isynoz_num)%pArray3D(:,:,:) < synoz_threshold)
            optDepth(:,:,:,5+3*N) = optDepth(:,:,:,5+3*N) + tArea(:,:,:,N+NSADdust)
         END WHERE
      ...
      IF ((AerDust_Effect_opt == 1) .or. (AerDust_Effect_opt == 3)) THEN
         DO R = 1,NRH_b
            ...
            odAer(:,:,:,R+4*NRH_b) = 0.d0  !SS(coarse1)
         ENDDO
         if(NSADaer.eq.7) then
             DO R = 1,NRH_b
                odAer(:,:,:,R+5*NRH_b) = 0.d0  !SS(coarse2)
                odAer(:,:,:,R+6*NRH_b) = 0.d0  !SS(coarse3)
             ENDDO
         endif
      END IF
      ...
      IF ((AerDust_Effect_opt == 2) .or. (AerDust_Effect_opt == 3)) THEN
         ...
         tArea(:,:,:,NSADdust+5) = 0.d0  !SS (coarse1)
         if(NSADaer.eq.7) then
            tArea(:,:,:,NSADdust+6) = 0.d0  !SS (coarse2)
            tArea(:,:,:,NSADdust+7) = 0.d0  !SS (coarse3)
         endif
      END IF
#endif
...
! !INTERFACE:
!
      subroutine Dust_OptDep_SurfArea(gridBoxHeight, &
