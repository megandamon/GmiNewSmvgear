...
!
     ...
     &                  month_gmi, jday, time_sec, londeg_i, latdeg_j, &
     &                  SZA_ij, tau_clw_ij, tau_cli_ij,                &
...
!
! !INPUT PARAMETERS:
      real*8, intent(in)  :: londeg_i                 ! first interface index
      real*8, intent(in)  :: latdeg_j                 ! first interface index
...
!
!      real*8, intent(in)  :: aeroProfile_ij(k1:k2,NSADaer) ! aerosol conc. [g/m2]
!      real*8, intent(in)  :: dustProfile_ij(k1:k2,NSADdust)! mineral dust conc. [g/m2]
!
...
!-----------------------------------------------------------------------
      ...
      call PHOTOJ(GMTAU, IDAY, ILNG, JLAT, SOLF, SZA_ij, U0, &
                  FREFL, ZPJ, loc_proc,londeg_i,latdeg_j)
...
!         ! Note that RAA has one dimension only.
!         bRAA(k,1:NAA) = RAA(1:NAA)
!
!         ! Get aerosol scattering phase functions
!         bQAA(k,1:NAA) = QAA(k,1:NAA)
!      end do
...
! Read in JPL spectral data and cloud-aerosol optical parameters
...
!---Zero / Set index arrays that map Jvalue(j) onto rates
      ...
      do J = 1, NJVAL  
         ...
         !if (TITLEJ(J) .eq. 'Acet-b') T_JX = 'C3H6Ob'
         do K = 1, NRATJ  
            T_CHEM = JLABEL (K)  
...
!<<<<<<<<<<<<<<<<<<<<<begin CTM-fastJX linking subroutines<<<<<<<<<<<<<<
!-----------------------------------------------------------------------
      subroutine PHOTOJ (UTIME, IDAY, ILNG, JLAT, SOLF, SZA_ij, &
                         U0, FREFL, ZPJ, loc_proc, londeg_i, latdeg_j)
!-----------------------------------------------------------------------
!
...
#     include "cmn_JVdat_fastJX65.h"  
      ...
      integer , intent (in) :: loc_proc
      real (8) , intent (in) :: londeg_i
      real (8) , intent (in) :: latdeg_j
...
!-----------------------------------------------------------------------
      ...
      call OPMIE (DTAUX, POMEGAX, U0, RFL, AMF2, JXTRA, AVGF, FJTOP, &
                   FJBOT, FSBOT, FJFLX, FLXD, FLXD0, loc_proc)
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      call JRATET (PPJ, TTJ, FFF, VALJL, loc_proc)  
!-----------------------------------------------------------------------
...
!######################################################################
!-----------------------------------------------------------------------
      subroutine JRATET (PPJ, TTJ, FFF, VALJL, loc_proc)  
!-----------------------------------------------------------------------
! in:
...
! endBian, 1/07/2012
         ...
         do J = 4, NJVAL  
            if (TQQ (2, J) .gt.TQQ (1, J) ) then  
               TFACT = max (0.d0, min (1.d0, (TT - TQQ (1,J) ) &
                     / (TQQ (2,J) - TQQ (1, J) ) ) )
            else  
               TFACT = 0.d0  
            endif  
...
! fast_JX approach
             TFACA = min(1.0, max(0.0, 19.4014-log10(DD) ))
             VALJ(NJVAL-1)  = VALJ(NJVAL-1) + &
                             (VALJ(NJVAL) - VALJ(NJVAL-1))*TFACA*1.9125
...
!         write(6,&
!     &      '(1x,i3,0p,f6.2,f10.3,f7.2,1p,4e9.2,0p,f10.4,2f8.5,2i3)') &
...
!-----------------------------------------------------------------------
! begBian, 11/22/2011
! replace subroutines RD_XXX and RD_MIE using previous GMI subroutine RD_TJPL
! add new features (e.g. truncate number of wavelengths to do 
!     troposphere-only) in RD_XXX and RD_MIE into RD_TJPL
! delete subroutine RD_UM
! endBian, 11/22/2011
...
!-----------------------------------------------------------------------
      subroutine OPMIE (DTAUX, POMEGAX, U0, RFL, AMF2, JXTRA, FJACT, &
             FJTOP, FJBOT, FSBOT, FJFLX, FLXD, FLXD0, loc_proc)
!-----------------------------------------------------------------------
      implicit none  
#     include "parm_CTM_fastJX65.h"  
#     include "parm_MIE_fastJX65.h"  
...
!---use inverse interpolation for correct tau-weighted values at edges
         do L2 = 3, L2_ - 1, 2  
            ...
            do I = 1, M2_  
               POMEGAJ (I,L2,K) = (POMEGAJ (I,L2-1,K) * TAUDN + POMEGAJ (I,L2+1,K) &
                                * TAUUP) / (TAUDN + TAUUP)
            enddo  
         enddo  
...
!---else scale by TAU
                  ...
                  do I = 1, M2_  
                     POMEGA (I, LZZ, K) = POMEGAB (I, K) + ATAUA &
                                     * (POMEGAJ (I,L2+1,K) - POMEGAB (I,K) )
                  enddo  
                  TAUBTM (K) = ZTAU (LZZ, K)  
                  FBTM (K) = FZ (LZZ, K)  
                  do I = 1, M2_  
...
!-----------------------------------------------------------------------
      ...
      call MIESCT (FJ, FJT, FJB, POMEGA, FZ, ZTAU, ZFLUX, RFL, U0, ND, loc_proc)  
!-----------------------------------------------------------------------
...
!---mean intensity averaged throughout layer:
         do L = 1, L_  
            ...
            SUMJ = (4.d0 * FJ (LZ0, K) + FZ (LZ0, K) ) * (ZTAU (LZ0 + 2, K) &
                 - ZTAU (LZ0, K) ) + (4.d0 * FJ (LZ1, K) + FZ (LZ1, K) ) &
                 * (ZTAU (LZ1, K) - ZTAU (LZ1 - 2, K) )
   
            SUMT = ZTAU (LZ0 + 2, K) - ZTAU (LZ0, K) + ZTAU (LZ1, K) &
                 - ZTAU (LZ1 - 2, K)
            do LZ = LZ0 + 2, LZ1 - 2, 2  
               SUMJ = SUMJ + (4.d0 * FJ (LZ, K) + FZ (LZ, K) ) &
...
!<<<<<<<<<<<<<<<<<<<<<<<begin core scattering subroutines<<<<<<<<<<<<<<<
!-----------------------------------------------------------------------
      subroutine MIESCT (FJ, FJT, FJB, POMEGA, FZ, ZTAU, ZFLUX, RFL, U0, ND, loc_proc)
!-----------------------------------------------------------------------
      implicit none  
...
#     include "parm_MIE_fastJX65.h"  
      ...
      real (8) , intent (in) :: ZTAU (N_, W_), RFL (W_), U0, ZFLUX (W_)
...
!---BLKSLV now called with all the wavelength arrays (K=1:W_)
      ...
      call BLKSLV (FJ, POMEGA, FZ, ZTAU, ZFLUX, RFL, PM, PM0, FJT, FJB, ND, loc_proc)
      return  
...
!######################################################################
!-----------------------------------------------------------------------
      subroutine BLKSLV (FJ, POMEGA, FZ, ZTAU, ZFLUX, RFL, PM, PM0, FJTOP, FJBOT, ND,loc_proc)
!-----------------------------------------------------------------------
...
!--- expect parameters M_, N_ in parm_MIE.f-----------------------------
      integer  , intent (in) :: ND  
      real (8) , intent (in) :: POMEGA (M2_,N_,W_), FZ (N_,W_)
      real (8) , intent (in) :: ZTAU (N_,W_), PM (M_,M2_), PM0 (M2_)
      real (8) , intent (in) :: RFL (W_), ZFLUX (W_)
...
!---------FINAL DEPTH POINT: L=ND
      ...
      do K = 1, W_  
         do J = 1, M_  
            do I = 1, M_  
               B (I,J,L,K) = B (I,J,L,K) + AA (I,1,L,K) * DD (1,J,L-1,K) &
                           + AA (I,2,L,K) * DD (2,J,L-1,K) + AA (I,3,L,K) &
                           * DD (3,J,L-1,K) + AA (I,4,L,K) * DD (4,J,L-1,K)
            enddo  
            H (J,L,K) = H (J,L,K) - AA (J,1,L,K) * RR (1,L-1,K) &
                      - AA (J,2,L,K) * RR (2,L-1,K) - AA (J,3,L,K) &
                      * RR (3,L-1,K) - AA (J,4,L,K) * RR (4,L - 1,K)
         enddo  
         do J = 1, M_  
            do I = 1, M_  
               F (K,I,J) = B (I,J,L,K)  
         ...
         do K = 1, W_  
            do J = 1, M_  
               RR (J,L,K) = RR (J,L,K) + DD (J,1,L,K) * RR (1,L + 1,K) &
                          + DD (J,2,L,K) * RR (2,L + 1,K) + DD (J,3,L,K) &
                          * RR (3,L+1,K) + DD (J,4,L,K) * RR (4,L + 1,K)
            enddo  
         enddo  
      ...
      do L = 2, ND, 2  
         do K = 1, W_  
            FJ (L,K) = RR (1,L,K) * WT (1) * EMU (1) + RR (2,L,K) &
                     * WT (2) * EMU (2) + RR (3,L,K) * WT (3) * EMU (3) &
                     + RR (4,L,K) * WT (4) * EMU (4)
         enddo  
      enddo  
