...
!   * April 4, 2013 - Stephen Steenrod
!     Modified the value of MX (from 35 to 45)
!-----------------------------------------------------------------------
      INTEGER      NB, NC
!jk      INTEGER      NS, NW, NP, NH, MX
!      PARAMETER   (NB=LPAR+1, NC=2*NB, NS=61, NW=18, NP=21, NH=7, MX=3)
!      PARAMETER   (NS=61, NW=18, NP=21, NH=7, MX=3)
!jk#ifdef GTmodule
!jk      PARAMETER   (NS=61, NW=30, NP=56, NH=7, MX=35)
!jk#else
!jk      PARAMETER   (NS=61, NW=18, NP=56, NH=7, MX=45)
!.sds      PARAMETER   (NS=61, NW=18, NP=56, NH=7, MX=35)
!jk#endif
!jk      INTEGER, parameter :: NB_MAX = LPAR_MAX+1   ! Added for GMI  {AOO, 8/04}
!jk      INTEGER, parameter :: NC_MAX = 2*NB_MAX     ! Added for GMI  {AOO, 8/04}
      ...
      real*8 FF1,FF2,FF3       !nika FF1=Flux@(TOA)*transmittance, FF2=Flux*(Transmittance^2)
     ...
     &              NJVAL,NW1,NW2,MIEDX(MX),NAA,NLBATM
      ...
      common/fluxes/FF1(NW,jpnl_max),FF2(NW,jpnl_max), FF3(NW,jpnl_max)
!-----------------------------------------------------------------------
