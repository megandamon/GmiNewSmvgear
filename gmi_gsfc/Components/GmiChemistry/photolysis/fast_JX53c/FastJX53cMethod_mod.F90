...
! !INPUT PARAMETERS:
      ...
      character (len=MAX_LENGTH_FILE_NAME), intent(in) :: CrossSection_InfileName
!                             ! fast-J X-sections (spectral data) input file name
      character (len=MAX_LENGTH_FILE_NAME), intent(in) :: ScatteringData_InfileName
!                             ! Aerosol/cloud scattering data input file name
      character (len=MAX_LENGTH_FILE_NAME), intent(in) :: PhotRateLabels_InfileName
!                             ! Labels of photolysis rates required input file name
!                             ! keyed to chem code
      character (len=MAX_LENGTH_FILE_NAME), intent(in) :: T_O3_Climatology_InfileName
...
! !INTERFACE:
!
      subroutine RunFastJX53c(k1, k2, IDAY, timeSec, iMONTH, lonDeg_i, latDeg_j, &
...
!-----------------------------------------------------------------------
      ...
      call PHOTOJ(GMTAU,IDAY,ILNG,JLAT, SolarZenithAngle, U0, FREFL, PhotRate)
