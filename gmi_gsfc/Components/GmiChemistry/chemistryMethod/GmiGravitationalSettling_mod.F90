...
!------------------------------------------------------------------------------
!BOC
      if (pr_diag) Write (6,*) 'updateGravitationalSettling called by ', loc_proc
     ...
     &               kel(i1:i2,ju1:j2,k1+1:k2)) * 0.5d0
      ...
      relhume(:,:,:) =  1013.25d0 * Exp (13.3185d0 * relhume(:,:,:)    -  &
     &                                    1.9760d0 * relhume(:,:,:)**2 -  &
     &                                    0.6445d0 * relhume(:,:,:)**3 -  &
     &                                    0.1299d0 * relhume(:,:,:)**4)

      relhume(:,:,:) =  (humidity(:,:,1:k2-1) + humidity(:,:,2:k2)) *  &
     &                   0.5d0 * MWTAIR / 18.0d0 /  &
     ...
     &             Sqrt (8.0d0 * MWTAIR / (GMI_PI * GAS_CONST_J * 1.0d3 * kele(:,:,:))))

#ifdef MICRO_AEROSOL
!-micro_aerosol--------begin---------------------------------------------
      !---------------------
      ! for adding umaerosol
      !---------------------

      ! compute wet radius of so4 aerosol
     ...
     &       concentration(INOC+ic-1)%pArray3D(:,:,k1+1:k2)) * 0.5d0
      ...
      ! non-so4 aerosol radius (m) and mass (kg/particle)
      do inon = 1, nnon
     ...
     &               * radvnon(inon)
      ...
      ! non-so4 aerosol number concentration (#particles/kg air)
      do inon = 1, nnon
         xnonum(:,:,:,inon) = aernon(:,:,:,inon) / pmsnon(inon)
      end do

      ! so4 volume on each non-so4 aerosol surface (m3/particle)
      do inon = 1, nnon
         so4vnon(:,:,:,inon) = max(1.0d-30,so4non(:,:,:,inon))  &
     &                       / max(r1,xnonum(:,:,:,inon))  &
...
!... don't settle lead or beryllium
        if (aerosol(ic) == 38 .or. aerosol(ic) == 39) then
           cycle ICLOOP
        endif
