...
!=============================================================================
!
! $Id: condense.F90,v 1.8 2011-08-09 22:12:58 mrdamon Exp $
!
! CODE DEVELOPERS
...
!                  (cm^-1)
!     nameOfModel: name of the model (assumed to be GEOS-5 if available)
!c    zmt        : zonal mean temperature (degK)
!
!   OUTPUT:
...
!c    real*8  :: zmt     (         ju1:j2,   k1:k2)
      ...
      CHARACTER(LEN=*), INTENT(IN) :: nameOfModel ! assumed to be GEOS-5
...
!cx   logical, parameter :: CALCSTS    = .true.
      logical, parameter :: CALCSTS    = .false.
      logical, parameter :: DOSEDIMENT = .true.

!c    real*8,  parameter :: PMAX = 200.0d0
!c    real*8,  parameter :: PMAX = 108.0d0
      real*8,  parameter :: PMIN =  10.0d0
!cx   real*8,  parameter :: TMAX = 210.0d0
...
!cx   real*8,  parameter :: SATRATICE =  1.4d0
...
!cx   real*8,  parameter :: NNAT      = 1.0d0
...
!     ----------------------
      ...
      integer                    :: numGlobLat
      real*8 , allocatable, save :: pstart(:)

      real*8  :: h2ogas
      real*8  :: pressmax
!c    real*8  :: rdum1, rdum2
      real*8  :: templat
...
!     ----------------
      ...
      if (pr_diag) then
        Write (6,*) 'Condense called by ', loc_proc
      end if

      numGlobLat = j2_gl - ju1_gl + 1   ! number of global latitudes

      IF (TRIM(nameOfModel) /= "GEOS-5") THEN
...
!        ==========
            ...
            allocate(pstart(ju1_gl:j2_gl))
            pstart(:) = 0.0d0

            !... more general code to limit psc calc to above tropopause 
            !    (latitude dependence)
            do ij = ju1_gl, j2_gl
               templat = abs(((real(ij)-1.d0)*(180.d0/(real(numGlobLat)-1.d0))-90.d0))
!               templat = abs((real(ij)*(180.d0/(real(numGlobLat)-1.d0))-90.d0))
               if (templat >= 50.d0) then
                  pstart(ij) = 120.d0
               elseif (templat <= 35.d0) then
                  pstart(ij) = 70.d0
               else
                  pstart(ij) = 70.d0+((120.d0-70.d0)*(templat-35.d0)/(50.d0-35.d0))
!                  pstart(ij) = 70.d0-((120.d0-70.d0)*(templat-35.d0)/(50.d0-35.d0))
               endif
            end do
         end if
...
!     ==================
            ...
            !---------------------------------------------------------------
            ! If predicted tropopause is available, it can be used for PMAX.
            !---------------------------------------------------------------

            IF (TRIM(nameOfModel) == "GEOS-5") THEN
               pressmax = tropp(il,ij)
            ELSE
               pressmax = pstart(ij)
            END IF
...
!           ---------------------------------------------------------
     ...
     &          (pres3c(il,ij,ik) <= pressmax) .and.  &
!c   &          (pres3c(il,ij,ik) <= PMAX)     .and.
     &          (pres3c(il,ij,ik) >= PMIN)) then

!              ===========
               call Sadice  &
!              ===========
     &           (temp3(il,ij,ik), pres3c(il,ij,ik),  &
     &            h2oback(il,ij,ik), dehyd(il,ij,ik),  &
     ...
     &            (h2ocond(il,ij,ik) == 0.0d0)) then
                ...
                h2ogas = h2oback(il,ij,ik) - dehyd(il,ij,ik)

!               ===========
                call Sadsts  &
!               ===========
!c   &            (zmt(ij,ik), temp3(il,ij,ik), pres3c(il,ij,ik),
     &            (temp3(il,ij,ik), pres3c(il,ij,ik),  &
!c   &             (h2oback(il,ij,ik)-dehyd(il,ij,ik)),
     &             h2ogas,  &
     &             hno3gas(il,ij,ik),  &
...
!       =============
     &    (dz, dt, temp3, pres3c,  &
     &     hno3cond, h2ocond, dehyd,  &
!c   &     hno3cond, h2ocond, h2oback, dehyd,
...
!c
!c             ---------------------------------------------------------
!c             Some time can be saved if calculations are not done when
!c             the temperature is too warm for condensed phases to form.
!c             Also, we can arbitrarily limit the pressures to those in
!c             the stratosphere where condensation is likely (i.e., the
!c             lower stratosphere).
!c             ---------------------------------------------------------
!c
!c             if ((temp3 (il,ij,ik) <= TMAX) .and.
!c      &          (pres3c(il,ij,ik) <= PMAX) .and.
!c      &          (pres3c(il,ij,ik) >= PMIN)) then
!c
!c                ===========
!c                call Sadice
!c                ===========
!c      &           (temp3(il,ij,ik), pres3c(il,ij,ik),
!c      &            h2oback(il,ij,ik), dehyd(il,ij,ik),
!c      &            h2ocond(il,ij,ik), rdum1,
!c      &            rdum2, CONSTANTNICE, SATRATICE,
!c      &            NICE, DENSICE, SIGICE, RICE)
!c
!c             end if
!c
!c           end do
...
!     =================
      ...
      subroutine sadice(temp3,pres3c,h2oback,dehyd,h2ocond,icesad,  &
...
! combine background and transported h2o

      h2oamb=h2oback-dehyd
!c
      h2oamb = Max (h2oamb, 0.5d-06)

! get partial pressure of h2o in pascal
! routine assumes h2oamb, h2oback, dehyd are given in mixing ratio

      presspasc=100.d0*pres3c
      h2opp=presspasc*h2oamb

! get dens using ideal gas law

!c    dens=6.02d23*presspasc/8.31d0/temp3/1.d6
      dens=AVOGAD*presspasc/GAS_CONST_J/temp3/1.d6

! saturation and nucleation temperature

      tsath2o=amm/(log10(h2opp)-bmm)
      tnuch2o=amm/(log10(h2opp/satratice)-bmm)

! if temp3 is below saturation temperature, calculate condensed h2o

      if(temp3.gt.tsath2o)then

         h2ocond=0.d0
         icesad=0.d0

      else if(temp3.gt.tnuch2o.and.h2ocond.eq.0.d0)then
         ...
         h2oeq=10.d0**(amm/temp3 + bmm)
         h2ocond=h2opp-h2oeq

! convert from pascal to number density (molecules/cm^3 air)
! first calculate local density using ideal gas law

         h2ocond=dens*h2ocond/presspasc
...
! convert h2o condensed phase number density to surface area density
      ...
      if(constantn)then
         ...
         icesad=(4.d0*pi*nice)**(1.d0/3.d0)*  &
     &          (3.d0*h2ocond/ndensice)**(2.d0/3.d0)  &
     &          *exp(-1.d0*logsigicesq)

         reffice=(3.d0*h2ocond/(ndensice*4.d0*pi*nice))**(1.d0/3.d0)  &
     &            *exp(-3.d0/2.d0*logsigicesq)



      else

         icesad=3.d0/ricecm*(h2ocond/ndensice)  &
     &          *exp(-5.d0*logsigicesq/2.d0)
...
!     =================
      ...
      subroutine sadnat(temp3,pres3c,h2ogas,h2ocond,hno3gas,hno3cond,  &
...
!c    dens=6.02d23*pres3c*100.d0/8.31d0/temp3/1.d6
...
!c
!c    h2ogas = Max (h2ogas, 0.5d-06)
!c    h2opp=presstorr*h2ogas
...
!c    tsathno3=(-b+sqrt(b*b-4*c))/2.d0
...
!c    tnuchno3=(-b+sqrt(b*b-4*c))/2.d0
...
!cx   else if(temp3.gt.tnuchno3.and.hno3cond.eq.0.d0)then
...
!     =================
      ...
      subroutine sadsts  &
!c   c          (zt,temp3,pres3c,h2ogas,h2ocond,hno3gas,hno3cond,
!c   c          (zt,temp3,pres3c,h2ogas,hno3gas,hno3cond,
...
!c    integer ilon,ilat,ivert

      logical constantn,constantnlbs

!c    real*8 temp3,pres3c,dens,hno3gas,h2ogas,
      real*8 temp3,pres3c,hno3gas,h2ogas,  &
!c   c h2so4gas,lbssad,h2ocond,hno3cond,nsts,sigsts,rsts,
     & h2so4gas,lbssad,hno3cond,nsts,sigsts,rsts,  &
!c   c rlbs,siglbs,nlbs,zt,h2oback
...
!c   c       ws,sadwn,v,parthno3,wcl,parthcl,whocl,hno3amb,
...
!c    call calcsulf(zt,pres3c,h2oback*1.d6,lbssad,h2so4gas,rlbs,siglbs,
      call calcsulf  &
...
! now call subroutine ternary:
      ...
      call ternary(temp3,pres3c,h2oppmv,hno3ppbv,h2so4ppbv,ws,sadwn,v,  &
!c   c     parthno3,saderr,denssts,ilon,ilat,ivert)
...
!     ===================
      ...
      subroutine calcsulf(t,ptot,qh2o,lbssad,h2so4gas,rlbs,siglbs,  &
     &                     nlbs,constantn)
...
!c             reff=(3.d0*v/(4.d0*pi*nlbs))**(1.d0/3.d0)
!c   c            *exp(-3.d0/2.d0*logsiglbssq)

            else

               v=rlbscm*lbssad/3.d0*  &
!c   c            *exp(5.d0*logsiglbssq/2.d0)
...
!c      tt=t
!c
!c      if(t.lt.tice-3.d0)tt=tice-3.d0
!c      if(tt.lt.185.d0)tt=185.d0
!c
...
!c    dens=6.02d23*ptot*100.d0/8.31d0/tt/1.d6
...
!     ==================
      ...
      subroutine ternary(tin,ptot,qh2o,qhno3,qh2so4,ws,sadwn,v,parthno3,  &
!c   c           saderr,denssts,ilon,ilat,ivert)
     &           saderr,denssts)
...
!c      real*8 ns,msb,mnb,ms,mn,mcl,mhocl
...
!c      real*8 t,tin,ptot,qh2o,qhno3,qhcl,qhocl,qh2so4
        real*8 t,tin,ptot,qh2o,qhno3,qh2so4

! dbc - declare currently undeclared internal variables

!c      real*8 pw,pn0,phcl0,phocl0,tt,tr,pr,tice,xsb,hsb,xnb,
        real*8 pw,pn0,tt,tr,pr,tice,xsb,hsb,xnb,  &
!c   c         hnb,a,b,c,phi,pi,pn,ws,sadwn,parthno3,v,w1,w2,hhcl,
     &         hnb,a,b,c,phi,pi,pn,ws,sadwn,parthno3,v,  &
!c   c         wcl,phcl,parthcl,hhocl,whocl,denssts,density
     &         denssts,density
...
! dbc - Input variables are passed to subroutine in subroutine
!       argument.  Delete input section here, but retain conversions
!       to internal variables (pw, etc.)

!        PRINT*, 'INPUT T '
!        READ(5,*) T
!
!        PRINT*, 'INPUT TOTAL PRESSURE IN MB'
!        READ(5,*) PTOT
!
!        PRINT*, 'INPUT H2O MIXING RATIO (PPMV) '
!        READ(5,*) QH2O

        PW=QH2O*1.d-6*PTOT/1013.0d0

!
!        PRINT*, 'INPUT MIXING RATIO OF HNO3 IN PPBV'
!        READ(5,*) QHNO3

        PN0=PTOT/1013.0d0*QHNO3*1.d-9

!
!        PRINT*, 'INPUT MIXING RATIO OF H2SO4 IN PPBV'
!        READ(5,*) QH2SO4
...
!        ns=qh2so4*1.d6/6.02d23
!
!        PRINT*, 'INPUT MIXING RATIO OF HCL IN PPBV '
!        READ(5,*) QHCL
!
!c?      qhcl = 0.0d0

!c      PHCL0=PTOT/1013.0d0*QHCL*1.d-9

!
!        PRINT*, 'INPUT MIXING RATIO OF HOCL IN PPBV '
!        READ(5,*) QHOCL
!
!c?      qhocl = 0.0d0

!c      PHOCL0=PTOT/1013.0d0*QHOCL*1.d-9
! ==========================================================================
!c      TT=R*T*NS
!c      TR=1.0d4/T-43.4782608d0
        PR=LOG(PW)+18.4d0
        TICE=2668.70d0/(10.4310d0-(LOG(PW)+LOG(760.0d0))/LOG(10.0d0))
!
...
! ASSUME SOLUTION IS PURE H2SO4/H2O
                 ...
                 PARTHNO3=1.0d0
              ENDIF
           V=NS*98.076d0/WS/DENSITY(MS,MN,T)*1.d6
...
! dbc - comment out print output section - output will occur through
!       subroutine call


!           PRINT*,'==================================================='
!           print*,'ns =',ns
!           print*,'sts density =',denssts
!           PRINT*,'T =',T,'K'
!           PRINT*,'WS =',WS
!           print*,'ms =',ms
!           PRINT*,'sadwn =',sadwn
!           print*,'mn =',mn
!           PRINT*,'PARTHNO3 =',PARTHNO3
!           PRINT*,'V =',V,'UM3/CM3'
...
!                 PRINT*,'WHCL',WCL,'PARTHCL',PARTHCL
...
! dbc delete printouts and include an error flag to determine if any
!     conditions have been violated

!           PRINT*,'TEMPERATURE MUST BE HIGHER THAN 3K BELOW THE ICE'
!           PRINT*,'       FROST POINT AND HIGHER THAN 185 K'
...
!c
        implicit none
!c
        real*8 cs,cn,t,denss,densn,density
...
!
     ...
     &       -9.703d-6*CN**2*T**2
        ...
        DENSITY=0.001d0/((1.0d0/DENSS*CS/(CS+CN)  &
     &  +1.0d0/DENSN*CN/(CS+CN)))

!        denstemp=0.001*denss*densn*(cs+cn)/(densn*cs+denss*cn)
...
#     include "gmi_phys_constants.h"
      ...
      real*8   hno3cond(i1:i2,   ju1:j2,   k1:k2)
      real*8   h2ocond (i1:i2,   ju1:j2,   k1:k2)
!c    real*8   h2oback (i1:i2,   ju1:j2,   k1:k2)
      real*8   dehyd   (i1:i2,   ju1:j2,   k1:k2)
      real*8   rsts, rnat
      real*8   rice
      real*8   reffsts (i1:i2,   ju1:j2,   k1:k2)
...
!c    if(maxht.eq.nvert)maxht=nvert-1
!c    if(minht.eq.1)minht=2
...
!c       dens=6.02d23*pres3c(il,ij,ik)*100.d0/8.31d0/
...
! any condensed h2o.
     ...
     &         fallvel(reffice(il,ij,ik)*1.d4,densice,  &
     &                 dens,temp3(il,ij,ik))

            else

               vfall(il,ij,ik)=fluxcorr*  &
     &         fallvel(rice,densice,dens,temp3(il,ij,ik))
     ...
     &            dens,temp3(il,ij,ik))
               ...
               if(constantnnat)then
                  ...
                  vfall(il,ij,ik)=fluxcorr*  &
     &            fallvel(reffnat(il,ij,ik)*1.d4,densnat,  &
     &            dens,temp3(il,ij,ik))

               else

                  vfall(il,ij,ik)=fluxcorr*  &
     &            fallvel(rnat,densnat,dens,temp3(il,ij,ik))
...
!c       idtsed=int(dt/dtsed)+1 ! number of iterations necessary
         fac    = dt / dtsed
         ifac   = fac
         idtsed = ifac + 1
!c
...
!         ---------------------------
          ...
          hno3cond(il,ij,ik) =  &
!c   &      (hno3cond(il,ij,ik) + (prod / dens)) / (1.0d0 + loss)
...
!c          prod1 = dtsed / dz(il,ij,ik) *
!c   &              Min (vfall(il,ij,ik), vfallmax)


!c          loss  = dtsed / dz(il,ij,ik) *
     ...
     &              h2ocond(il,ij,ik+1) * densp
            ...
            h2ocond(il,ij,ik) =  &
!c   &        (h2ocond(il,ij,ik) + (loss / dens)) / (1.0d0 + prod1)
...
!           ------------------------------------------------------------
!           Production of dehydration is a loss for h2ocond.  This is an
!           explicit update to allow for layer moistening (negative
!           values) of dehyd.  It is necessary to make sure that dehyd
!           never gets larger than h2oback.
!           ------------------------------------------------------------

!c?         if (dehyd(il,ij,ik) .gt. h2oback(il,ij,ik)) then
!c            dehyd(il,ij,ik) = h2oback(il,ij,ik)
!c          end if

!c          if (dehyd(il,ij,ik) .gt. h2oback(il,ij,ik)-1.0d-06) then
!c            dehyd(il,ij,ik) = h2oback(il,ij,ik)-1.0d-06
!c          end if
...
! output.
         ...
         fallvel=0.2222d0*rho*radius*radius*grav/dynvis*100.d0*  &
