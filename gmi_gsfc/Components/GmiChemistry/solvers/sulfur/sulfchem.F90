!=============================================================================
!
! $Id: sulfchem.F90,v 1.5 2013-09-03 16:16:43 jkouatch Exp $
!
! CODE DEVELOPER
...
!  - January 2011 Steve Steenrod
!     Adapted for use with coupled gas-aerosol chemistry
!=============================================================================
...
!-----------------------------------------------------------------------------
     ...
     &   dms_oh, dms_no3, so2_oh, so2_h2o2, so2_o3, chem_opt)
...
!... used to control things for full chem with sulfur
...
!     *****************************************************************
!.sds... need to load proper h2o2 photolysis rate - NEED TO GENERALIZE!!!!
      if(chem_opt == 8) then
        do ijk = 1, itloop
          qj4p(ijk) = qjgmi(ijk, 1)
        end do
      else if(chem_opt == 2) then
          qj4p(:) = 0.0
      endif
...
!          =================
!    &  (sulfSavedVars, itloop, pgcm, tgcm,
...
!     *****************************************************************
        ...
        call Do_Setup_Aquchem (itloop, numcl, idxcl, cfac, cwac)
...
!.sds... zero out gas phase reaction rates for full chem - done in smvgear
      if(chem_opt == 2) then
        acoef(:,1) = 0.0    ! DMS + OH -> SO2
        acoef(:,2) = 0.0    ! SO2 + OH -> H2SO4
        acoef(:,9) = 0.0    ! DMS + NO3 -> SO2
        acoef(:,3) = 0.0    ! HO2 + HO2 -> H2O2
        acoef(:,4) = 0.0    ! H2O2 + hv -> OH + OH
        acoef(:,5) = 0.0    ! H2O2 + OH -> H2O + HO2
        acoef(:,8) = 0.0    ! HO2 + HO2 + H2O -> H2O2 + H2O + O2
      endif

!.sds. if statement added, not code within
!... do gas phase chemistry only if running gocart (not coupled aerosol chemistry)
      if(chem_opt == 8) then
!     ***********************************************************
...
!... in gocart h2o2 can get very high near polar terminator in upper strat/lower meso - cap it at 1 ppm
          if( any(cp(:,IH2O2) > cp(:,IMGAS)*1.0d-6) ) then 
            print '(''WARNING- gas H2O2 too big - no. pts='',i8,'' - processor'',i4)' &
                  , count(cp(:,IH2O2) > cp(:,IMGAS)*1.0d-6), loc_proc
            where(cp(:,IH2O2) > cp(:,IMGAS)*1.0d-6) cp(:,IH2O2) = cp(:,IMGAS)*1.0d-6
          endif
