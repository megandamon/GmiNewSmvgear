!=============================================================================
!
! $Id: sulfchem_solver.F90,v 1.8 2013-09-03 16:16:43 jkouatch Exp $
!
! CODE DEVELOPER
!   Xiaohong Liu
!
...
!-----------------------------------------------------------------------------
     ...
     &  (sulfSavedVars, time, nymd, nhms, tempc_nb, presc_nb, const_nb, humidity,  &
...
!     ----------------------
      ...
      real*8 :: qjgmi_saved(i1:i2, ju1:j2, k1:k2)
...
!        Interpolation of qj (91x5) to the GMIMOD pressure levels.
     ...
     &           + (presc_nb(il,ij,ik) * 1.0d-3 - 0.1d0) / 0.3d0)
               ...
               qjgmi(il,ij,ik,1) =  &
     &               sulfSavedVars%qjh2o2_2d(ij,k,im)  &
     &               + (sulfSavedVars%qjh2o2_2d(ij,k+1,im)&
                     -  sulfSavedVars%qjh2o2_2d(ij,k,im))  &
...
!.sds. if statement added, not the code within
!... do diurnal cycle to OH, etc only if running gocart (not full chem with sulfur)
      if(chem_opt == 8) then
        ...
        qjgmi_saved = qjgmi(i1:i2,ju1:j2,k1:k2,1)
...
!     =====================
     &    (sulfSavedVars%lon_dummy, nymd, nhms, londeg, latdeg, &
           hox_qj_scaler, no3_scaler, &
     ...
     &          const_nb(il,ij,ik,IHO2) * hox_qj_scaler(il,ij)
              qjgmi(il,ij,ik,1) =  &
     &          qjgmi(il,ij,ik,1) * hox_qj_scaler(il,ij)
...
!... do gas phase only of running gocart (not full chem with sulfur)
      if(chem_opt == 8) then
        ...
        qjgmi   (i1:i2,ju1:j2,k1:k2,1   ) = qjgmi_saved
#endif
      endif
...
!-----------------------------------------------------------------------------
     ...
     &  (lon_dummy, nymd, nhms, londeg, latdeg, hox_qj_scaler, &
         no3_scaler, i1, i2, ju1, j2, i1_gl, i2_gl, ju1_gl, j2_gl, ilong)
...
!     ----------------------
      ...
      real*8  :: hox_qj_scaler(i1:i2, ju1:j2)
      real*8  :: no3_scaler   (i1:i2, ju1:j2)
      real*8, intent(inOut) :: lon_dummy(1)
