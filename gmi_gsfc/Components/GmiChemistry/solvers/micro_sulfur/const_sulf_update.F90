!=============================================================================
!
! $Id: const_sulf_update.F90,v 1.2 2011-08-09 22:12:58 mrdamon Exp $
!
! CODE DEVELOPER
...
!     cp(:, ISO4M1) = max(cp(:, ISO4N1)*r2so4min, cp(:, ISO4M1))
!     cp(:, ISO4M2) = max(cp(:, ISO4N2)*r2so4min, cp(:, ISO4M2))
!     cp(:, ISO4N1) = max(cp(:, ISO4M1)/rso4max,  cp(:, ISO4N1))
!     cp(:, ISO4N2) = max(cp(:, ISO4M2)/rso4max,  cp(:, ISO4N2))
