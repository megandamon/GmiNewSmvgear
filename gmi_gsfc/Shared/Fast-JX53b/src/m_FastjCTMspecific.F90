...
! !USES:
!
      use m_FastjMetVars      , only : T, P, PMEAN, TJ, DM
      use m_FastjMetVars      , only : ETAA, ETAB
      use m_FastjMetVars      , only : ZH, STT, DO3, AREAXY, TCNAME
      use m_FastjCTMparameters, only : I_, J_, L_, L1_, NTR_
!
      implicit none
!
! !INPUT PARAMETERS:
...
! !USES:
!
      use m_FastjCoreFastj, only : RD_XXX, RD_MIE
!
      implicit none
!
! !INPUT PARAMETERS:
...
! !USES:
!
      use m_FastjJvaluesVars  , only : JFACTA, JLABEL, TITLEJ
      use m_FastjJvaluesVars  , only : JIND, NRATJ, NJVAL
      use m_FastjCTMparameters, only : JVN_
!
      implicit none
!
!
...
! !USES:
!
      use m_FastjMetVars      , only : OREF, TREF
!
      implicit none
!
! !INPUT PARAMETERS:
...
! !USES:
!
      use m_FastjMetVars      , only : T, ODCLD, SA, NCLDX
      use m_FastjCTMparameters, only : I_, J_, L_, L1_
!
      implicit none
!
! !INPUT PARAMETERS:
...
!
! !USES:
      use m_FastjMetVars      , only : NAER1, NAER2, NAER3
      use m_FastjMetVars      , only : DAER1, DAER2, DAER3
      use m_FastjCTMparameters, only : I_, J_, L_
!
      implicit none
!
! !DESCRIPTION:
...
!
! !USES:
      use m_FastjMetVars      , only : OREF, TREF, DM, DO3, TJ, ZH
      use m_FastjMetVars      , only : ETAA, ETAB, PMEAN, MONTH
