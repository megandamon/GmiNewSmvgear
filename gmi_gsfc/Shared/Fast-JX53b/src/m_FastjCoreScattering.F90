...
! !INTERFACE:
!
      subroutine MIESCT(FJ,POMEGA,FZ,ZTAU,ZFLUX,ZREFL,ZU0,MFIT,ND)
!
! !USES:
...
!
!   SUBROUTINES:  MIESCT              needs 'parm_MIE.f'
!                 BLKSLV              needs 'parm_MIE.f'
!                 GEN (ID)            needs 'parm_MIE.f'
!                 LEGND0 (X,PL,N)
!                 MATIN4 (A)
!                 GAUSSP (N,XPT,XWT)
! \end{verbatim}
...
!-------------------------------------------------------------------------
      ...
      end module m_FastjCoreScattering
