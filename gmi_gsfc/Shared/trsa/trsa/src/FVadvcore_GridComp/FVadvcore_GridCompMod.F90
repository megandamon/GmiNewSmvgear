! $Id: FVadvcore_GridCompMod.F90,v 1.2 2011-08-09 22:13:00 mrdamon Exp $
...
!   WS 2008_12_05: changed nsplit default from 1 to 10.  Safer.
    call MAPL_GetResource(MAPL, NS  , 'nsplit:', default=10,     rc=STATUS)
    VERIFY_(STATUS)
...
!------------------------------------------------
    ...
    print *, "Minval DP", minval(DP), "Maxval(DP)", maxval(DP)
...
!!!    PE2(:,:,0) = PTOP
    ...
    print *, "min(DP1)", minval(DP1(:,:,:)), "min(DP)", minval(DP(:,:,:))
    do K=1,LM
       PE1(:,:,K) = PE1(:,:,K-1) + DP1(:,:,K)
!!!       PE2(:,:,K) = PE2(:,:,K-1) + DP(:,:,K)
    enddo
...
!
!!!    if(FRIENDLY==ESMF_true) then
!!!       DP = DP1   ! WHY SHOULD THIS BE SET TO DP1?
!!!    end if
