! $Id: ESMFL_Mod.F90,v 1.2 2011-08-09 22:12:59 mrdamon Exp $
...
!
  ...
  interface ESMFL_StateGetPointerToData
     module procedure ESMFL_StateGetPointerToData1D
     module procedure ESMFL_StateGetPointerToData2D
     module procedure ESMFL_StateGetPointerToData3D
     module procedure ESMFL_StateGetPointerToData4D
     module procedure ESMFL_StateGetPtrToData1D_R8
     module procedure ESMFL_StateGetPtrToData2D_R8
     module procedure ESMFL_StateGetPtrToData3D_R8
     module procedure ESMFL_StateGetPtrToData4D_R8
  end interface
...
!ALT   VERIFY_(STATUS)
 ...
 subroutine ESMFL_StateGetField(state, fieldname, bundle, fieldalias, RC)
   ...
   type(ESMF_Grid)            :: grid
   type(ESMF_Array)           :: array, array1
   type(ESMF_FieldDataMap)    :: datamap           
   type(ESMF_RelLoc)          :: relloc
   integer                    :: COUNTS(3), rank, DIMS
   integer                    :: status, I
...
!  since it does not know what grid to use in the dummy.
   ...
   do i=1,size(FIELDNAME)
      ...
      if(present(fieldalias)) then
         call ESMF_StateGetField (STATE, trim(FIELDNAME (I)), FIELD , rc=STATUS)
         VERIFY_(STATUS)
         call ESMF_BundleGetField(BUNDLE,trim(FIELDALIAS(I)), FIELDB, rc=STATUS)
         VERIFY_(STATUS)
         call ESMF_FieldGetArray (Field , ARRAY, RC=STATUS)
         VERIFY_(STATUS)
         call ESMF_FieldSetArray (FieldB, ARRAY, RC=STATUS)
         VERIFY_(STATUS)
      ...
      call ESMF_StateGetField(STATE, trim(FIELDNAME(I)), FIELD, rc=STATUS)
      if(STATUS/=ESMF_SUCCESS) then
         call ESMF_BundleGetField (BUNDLE, 1, FIELD, RC=STATUS)
         VERIFY_(STATUS)
         call ESMF_FieldGet (FIELD, GRID=GRID, ARRAY=Array1, datamap=datamap, &
                             horzRelLoc=RELLOC, RC=STATUS)

         VERIFY_(STATUS)

         call ESMF_ArrayGet(Array1, RANK=rank, COUNTS=counts, RC=status)
         VERIFY_(STATUS)
         ARRAY = ESMF_ArrayCreate(RANK, ESMF_DATA_REAL, ESMF_R4, &
              COUNTS, RC=STATUS )
         VERIFY_(STATUS)
         call MAPL_ArrayF90Deallocate(array, rc=status) ! empty field
         ...
         FIELD = ESMF_FieldCreate(GRID, ARRAY, &
              ...
              datamap=datamap, &
              name=trim(FIELDNAME(I)),rc=STATUS)
         VERIFY_(STATUS)
...
! ! INTERFACE ESMFL_GridCoordGet - retrieves the coordinates of a particular axis
 !EOP
subroutine ESMFL_GridCoordGet(GRID, coord, name, Location, Units, rc)
   type(ESMF_Grid),   intent(IN ) :: GRID
   real, dimension(:,:), pointer  :: coord
   character (len=*) , intent(IN) :: name
   type(ESMF_RelLoc)              :: location
   integer                        :: units
   integer, optional              :: rc
...
!	local variables
  ...
  call ESMF_GridGetCoord(GRID, horzRelloc=Location, centerCoord=earray, rc=status) 
  VERIFY_(STATUS) 
  ...
  call ESMF_ArrayGet(earray(crdOrder), RANK=rank, TYPE=type, KIND=kind, &
       COUNTS=counts, RC=status)
  VERIFY_(STATUS)

  if (rank /=2 ) then ! ALT: for now 
   STATUS=ESMF_FAILURE
   VERIFY_(STATUS)
  endif
  if (type /= ESMF_DATA_REAL) then 
     STATUS=ESMF_FAILURE
     VERIFY_(STATUS)
  endif
  ...
  if (kind == ESMF_R8) then 
     call ESMF_ArrayGetData(earray(crdOrder), r8d2, RC=status)
     VERIFY_(STATUS)
     coord = r8d2

  else
!!! ALT: This here is an ugly kludge. Replace ASAP
...
! End of kludge
 ...
 subroutine ESMFL_StateFreePointers(STATE, RC)
   ...
   real, pointer                    :: PTR3(:,:,:)
   real, pointer                    :: PTR4(:,:,:,:)
   type(ESMF_logical)               :: NEEDED

   character (len=ESMF_MAXSTR), pointer :: ITEMNAMELIST(:)
   type(ESMF_StateItemType)   , pointer :: ITEMTYPELIST(:)
...
!---------------------------
   ...
   call ESMF_StateGet(STATE,ITEMNAMELIST=ITEMNAMELIST,STATEITEMTYPELIST=ITEMTYPELIST,RC=STATUS)
   VERIFY_(STATUS)

   do I=1,ITEMCOUNT
      if(ITEMTYPELIST(I)==ESMF_STATEITEM_FIELD) then
         call ESMF_StateGetField(STATE, trim(ITEMNAMELIST(I)), FIELD, RC=STATUS)
         VERIFY_(STATUS)

         call ESMF_FieldGetAttribute  (FIELD, NAME="Needed",VALUE=NEEDED, RC=STATUS)
         if(STATUS /= ESMF_SUCCESS) NEEDED = ESMF_false

         if(NEEDED==ESMF_false) then
            call ESMF_FieldGetArray(FIELD, ARRAY,       RC=STATUS)
            VERIFY_(STATUS)
            ...
            select case (rank)
            case (1)
               call ESMF_ArrayGetData (array, PTR1, RC=status)
               VERIFY_(STATUS)
            ...
            case (4)
               call ESMF_ArrayGetData (array, PTR4, RC=status)
               VERIFY_(STATUS)
 ...
 subroutine ESMFL_StateSetFieldNeeded(STATE, NAME, RC)
   ...
   call ESMF_StateGetField(STATE, trim(NAME), FIELD, RC=STATUS)
   VERIFY_(STATUS)

   call ESMF_FieldSetAttribute  (FIELD, NAME="Needed",VALUE=ESMF_false, RC=STATUS)
   VERIFY_(STATUS)
...
function ESMFL_StateFieldIsNeeded(STATE, NAME, RC) result(NEEDED)
 ...
 subroutine ESMFL_StateGetPointerToData1D(state, PTR, NAME, alloc, notFoundOK, RC)
   type(ESMF_State),  intent(INOUT) :: state
   ...
   character(len=ESMF_MAXSTR)      :: IAm="ESMFL_StateGetPointerToData1D"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_FieldGetArray(field, array, rc=status)
   VERIFY_(STATUS)
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      COUNTS=counts, LBOUNDS=lbounds, UBOUNDS=ubounds, &
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 1 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R4) then
      RETURN_(ESMF_FAILURE)
   end if

   call ESMF_ArrayGetData(array, PTR, RC=status)
   VERIFY_(STATUS)
   if (present(alloc)) then
      if (alloc .and. (.not. associated(PTR))) then
         call MAPL_AllocateCoupling(array, rc=STATUS)
         VERIFY_(STATUS)

         if(.not. ESMF_StateIsNeeded(STATE, NAME, RC=STATUS)) then
            VERIFY_(STATUS)
            call ESMF_StateSetNeeded(STATE, NAME, &
 ...
 end subroutine ESMFL_StateGetPointerToData1D

 subroutine ESMFL_StateGetPointerToData2D(state, PTR, NAME, alloc, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R4), pointer :: PTR(:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)     :: IAm="ESMFL_StateGetPointerToData2D"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 2 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R4) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPointerToData2D

 subroutine ESMFL_StateGetPointerToData3D(state, PTR, NAME, alloc, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R4), pointer :: PTR(:,:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)     :: IAm="ESMFL_StateGetPointerToData3D"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 3 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R4) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPointerToData3D

 subroutine ESMFL_StateGetPointerToData4D(state, PTR, NAME, alloc, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R4), pointer :: PTR(:,:,:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)     :: IAm="ESMFL_StateGetPointerToData4D"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 4 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R4) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPointerToData4D

 subroutine ESMFL_StateGetPtrToData1D_R8(state, PTR, NAME, alloc, notFoundOK, RC)
   type(ESMF_State),  intent(INOUT) :: state
   ...
   character(len=ESMF_MAXSTR)      :: IAm="ESMFL_StateGetPtrToData1D_R8"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 1 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R8) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPtrToData1D_R8

 subroutine ESMFL_StateGetPtrToData2D_R8(state, PTR, NAME, alloc, notFoundOK, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R8), pointer :: PTR(:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)      :: IAm="ESMFL_StateGetPtrToData2D_R8"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 2 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R8) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPtrToData2D_R8

 subroutine ESMFL_StateGetPtrToData3D_R8(state, PTR, NAME, alloc, notFoundOK, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R8), pointer :: PTR(:,:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)      :: IAm="ESMFL_StateGetPtrToData3D_R8"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
   ...
   call ESMF_ArrayGet(array, RANK=rank, TYPE=type, KIND=kind, &
                      ...
                      RC=status)
   VERIFY_(STATUS)
   if (RANK /= 3 .OR. type /= ESMF_DATA_REAL .OR. kind /= ESMF_R8) then
      RETURN_(ESMF_FAILURE)
   end if
 ...
 end subroutine ESMFL_StateGetPtrToData3D_R8

 subroutine ESMFL_StateGetPtrToData4D_R8(state, PTR, NAME, alloc, notFoundOK, RC)
   type(ESMF_State),  intent(INOUT) :: state
   real(KIND=ESMF_KIND_R8), pointer :: PTR(:,:,:,:)
   character(len=*),  intent(IN   ) :: NAME
   logical, optional, intent(IN   ) :: alloc
   ...
   character(len=ESMF_MAXSTR)      :: IAm="ESMFL_StateGetPtrToData4D_R8"
   integer                         :: rank, status
   type (ESMF_Array)               :: array
   type (ESMF_Field)               :: field
   type(ESMF_DataType)             :: type
 ...
 end subroutine ESMFL_StateGetPtrToData4D_R8
   ...
   subroutine ESMFL_BundleGetPointerByIndex2(BUNDLE,INDEX,PTR,RC)
     ...
     call ESMF_BundleGetField (BUNDLE, INDEX, FIELD, RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_FieldGetArray  (FIELD, ARRAY,             RC=STATUS)
     if ( STATUS /= ESMF_SUCCESS) THEN
	NULLIFY(PTR);
	RETURN_(ESMF_SUCCESS)
     ...
     call ESMF_ArrayGetData   (ARRAY,PTR, ESMF_DATA_REF, RC=STATUS)
     VERIFY_(STATUS)
   ...
   subroutine ESMFL_BundleGetPointerByName2(BUNDLE,NAME,PTR,RC)
     ...
     call ESMF_BundleGetField (BUNDLE, NAME, FIELD,     RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_FieldGetArray  (FIELD, ARRAY,             RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_ArrayGetData   (ARRAY,PTR, ESMF_DATA_REF, RC=STATUS)
   ...
   subroutine ESMFL_BundleGetPointerByName3(BUNDLE,NAME,PTR,RC)
    ...
    subroutine ESMFL_BundleCpyField (BUNDLE, FIELD, NAME, RC)
     ...
     call ESMF_FieldGet(FIELD, datamap    =DATAMAP,&
                               horzRelLoc =RELLOC, &
                               array      =ARRAY,  &
                               grid       =GRID,   &
                                         RC=STATUS )
     ...
     FIELD1 = ESMF_FieldCreate(GRID, ARRAY,            &
              datamap    = datamap,                    &
              horzRelloc = relloc,                     &
              name       = name,             RC=STATUS )
     VERIFY_(STATUS)
 
     call ESMF_BundleAddField (BUNDLE, FIELD1, RC=STATUS )
     VERIFY_(STATUS)
...
! local vars
   ...
   call ESMF_StateGetField(STATE, NAME, F, RC=STATUS)
   VERIFY_(STATUS)
...
! local vars
    ...
    call ESMF_GridGetDELocalAI(grid, horzRelLoc=ESMF_CELL_CENTER, AIPerDim=ai_global, rc=status)
...
! local vars
   ...
   real(4), pointer        :: Sptr2d(:,:)
   real(4), pointer        :: Dptr2d(:,:)
   real                    :: pi
...
! local vars
   ...
   type(ESMF_Grid)         :: grid3D
   type(ESMF_FieldDataMap) :: dmap
   real(4), dimension(:,:), pointer   :: sptr, dptr
   real(4), dimension(:,:), pointer   :: sptr2d, dptr2d
   real(4), dimension(:,:,:), pointer :: sptr3d, dptr3d
   integer :: sCPD(3), dCPD(3) ! localCellCountPerDim
...
! ! ARGUMENTS:
   ...
   type(ESMF_Bundle), intent(in)         :: srcBUN
   type(ESMF_Bundle), intent(inout)      :: dstBUN
   type(ESMF_Grid), intent(in)           :: Sgrid2D 
   type(ESMF_Grid), intent(in)           :: Dgrid2D
   type(ESMF_RouteHandle), intent(inout) :: rh
   type(ESMF_VM), intent(inout)          :: vm
...
! local vars
   ...
   real(4), dimension(:,:), pointer   :: sptr2d, dptr2d
   real(4), dimension(:,:,:), pointer :: sptr3d, dptr3d
   integer :: sCPD(3), dCPD(3), gccpd(3)
   integer :: rank, k, deid, kmax, status, L, numVars
   character(len=ESMF_MAXSTR) :: srcName, dstName, fname
...
! begin
   ...
   call ESMF_BundleGet      (srcBUN, FieldCount=NumVars,  RC=STATUS)
   VERIFY_(STATUS)

   call ESMF_BundleGetField (srcBUN,  1, srcFLD,            RC=STATUS)
   VERIFY_(STATUS)
   call ESMF_BundleGetField (dstBUN,  1, dstFLD,            RC=STATUS)
   VERIFY_(STATUS)
   ...
   do L=1,NumVars   
     ...
     call ESMF_BundleGetField     (srcBUN, L, srcFLD,           RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_FieldGet           (srcFLD, NAME=srcName,         RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_BundleGetField     (dstBUN, L, dstFLD,           RC=STATUS)
     VERIFY_(STATUS)
     ...
     ! note halo width = 0
     if(rank==3) then
       call ESMF_FieldGetDataPointer (srcFLD, sptr3d, rc = status)
       VERIFY_(status)
       call ESMF_FieldGetDataPointer (dstFLD, dptr3d, rc = status)
       VERIFY_(status)
     else
...
! ! ARGUMENTS:
   ...
   type(ESMF_Bundle), intent(inout)      :: srcBUN ! source bundle
   type(ESMF_Bundle), intent(inout)      :: dstBUN ! destination bundle
...
!     a PE determined by a slice-to-PE map (slice_pe) to "load balance"
...
! local vars
   ...
   real(4), allocatable, dimension(:,:,:) :: srcBUF  ! src buffer
   real(4), allocatable, dimension(:,:,:) :: dstBUF  ! dst buffer
   real(4),     pointer, dimension(:,:  ) :: llons, llats 
   real(4), allocatable, dimension(:,:  ) :: glons, glats 
   real(4), allocatable, dimension(:    ) :: srcLons, srcLats 
   real(4), allocatable, dimension(:,:  ) :: srcWork 
   real(4), allocatable, dimension(:,:  ) :: dstWork
   character(len=ESMF_MAXSTR), parameter  :: IAm = 'BundleRegrid'
...
! get lons,lats used by hhinterp
   ...
   call ESMF_VMBroadcast(vm, srcLats, jms_world, ROOT, rc=status)
   ...
   call ESMF_VMBroadcast(vm, srcLons, ims_world, ROOT, rc=status)

   call ESMF_GridGetAttribute(dstGrid, 'FLIP_LONS', flip_lons, rc=status)
   if (status /= ESMF_SUCCESS) flip_lons = ESMF_FALSE
   call ESMF_GridGetAttribute(dstGrid, 'FLIP_POLES', flip_poles, rc=status)
   if (status /= ESMF_SUCCESS) flip_poles = ESMF_FALSE
   if(mype==ROOT) then
      if(flip_lons==ESMF_TRUE) print *, trim(Iam)//': We will flip lons'
      if(flip_poles==ESMF_TRUE) print *, trim(Iam)//': We will flip poles'
   end if
...
! ! ARGUMENTS:
   ...
   type(ESMF_Bundle), intent(inout)         :: BUN
...
! locals
   ...
   real(4), dimension(:,:  ), pointer    :: ptr2d
   real(4), dimension(:,:,:), pointer    :: ptr3d
   character(len=ESMF_MAXSTR)            :: name
...
! start
   ...
   do n = 1, NumVars
     ...
     call ESMF_BundleGetField (BUN, n, FLD, RC=STATUS)
     if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam)
     ! get field name
     ...
     hw = 0
     call ESMF_FieldGetAttribute(FLD, "HALOWIDTH", halowidth, &
                                 rc=status)
     if (status == ESMF_SUCCESS) hw = halowidth
     if (mype==ROOT .and. n==1) print *, ' halowidth = ',hw

     if (rank==2) then
       call ESMF_FieldGetDataPointer (FLD, ptr2d, rc = status)
       if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam)
     else
       call ESMF_FieldGetDataPointer (FLD, ptr3d, rc = status)
       if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam)
     end if    
     ...
     do L = 1, lmFLD
       ...
       if (rank==2) then
          ...
          if(hw>0) then
             ! start at L=2 since z-size of ptr3d if lmFLD+2
             call ArrayGather(ptr3d(:,:,L+1), srcWork, srcGRID, &
                  depe = slice_pe(ns), hw=hw, RC=STATUS)
          else
...
! locals
 ...
 ! start
   ...
   if (flip_lons  == ESMF_TRUE) call FlipLons_ (outBuf)
   if (flip_poles == ESMF_TRUE) call FlipPoles_(outBuf)
   ...
   do n = 1, NumVars
     ...
     do L = 1, lmFLD
       ...
       if (rank==2) then
          ...
          if(hw>0) then
             ! start at L=2 since z-size of ptr3d if lmFLD+2
             call ArrayScatter(ptr3d(:,:,L+1), dstWork, dstGRID, &
                  depe=slice_pe(ns), hw=hw, RC=STATUS)
          else
...
!-------------------------------------------------------------------------
     ...
     real(4),allocatable, dimension(:,:) :: sfcin
     integer :: im,jm
...
! locals
   ...
   type(ESMF_Field)           :: FLD
   type(ESMF_Grid)            :: GRID
   type(ESMF_Bundle)          :: srcBUN
   type(ESMF_Bundle)          :: dstBUN
   integer :: status, mype, npe, i, k, itemcount
   character (len=ESMF_MAXSTR), pointer  :: itemnamelist(:)
   type(ESMF_StateItemType)   , pointer  :: itemtypelist(:)
   character(len=ESMF_MAXSTR)            :: name 
...
! than a field then return(1)
   ...
   call ESMF_StateGet(srcSTA, itemnamelist=itemnamelist, &
                      stateitemtypelist=itemtypelist, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam)
...
! get grid from first field in state, and create bundle
   ...
   if(mype==ROOT) print *, ' Get grid from ',trim(itemnamelist(1))
   call ESMF_StateGetField(srcSTA, trim(itemnamelist(1)), FLD, &
                           rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGetField failed')
   call ESMF_StateGet(srcSTA, name, rc=status)
   ...
   srcBUN = ESMF_BundleCreate ( name=name, &
                             grid=GRID, rc=status )
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' src ESMF_BundleCreate failed' )
...
! get grid from first field in state, and create bundle
   ...
   if(mype==ROOT) print *, ' Get grid from ',trim(itemnamelist(1))
   call ESMF_StateGetField(dstSTA, trim(itemnamelist(1)), FLD, &
                            rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGetField failed')
   call ESMF_StateGet(dstSTA, name, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGet failed')
   name = name//'to bundle'
   call ESMF_FieldGet (FLD, grid=GRID, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_FieldGet failed')
   ! create bundle with input grid
   dstBUN = ESMF_BundleCreate ( name=name, &
                             grid=GRID, rc=status )
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_BundleCreate failed' )    
...
! start
   ...
   call ESMF_FieldGetArray(FLD, ARR, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam)
...
! ! ARGUMENTS:
   ...
   type(ESMF_Bundle), intent(inout) :: srcBUN
   type(ESMF_Bundle), intent(inout) :: dstBUN
   integer, optional, intent(out)   :: rc     ! return code
...
! local vars
   ...
   real(4), dimension(:,:  ), pointer   :: sptr2d, dptr2d
   real(4), dimension(:,:,:), pointer   :: sptr3d, dptr3d
   real(4), allocatable, dimension(:,:) :: srcBuf, dstBuf, diff
   integer :: rank, i, j, k, mype, kmax, status, L, numVars
   integer :: globalCPD(3)
   character(len=ESMF_MAXSTR)            :: srcName
   character(len=ESMF_MAXSTR)            :: dstName
   character(len=ESMF_MAXSTR)            :: peinf
...
! begin
   ...
   if(mype==ROOT) print *,' Loop through bundles'
...
!-------------------------------------------------------------------------
   subroutine State2Bundle (STA, BUN, rc)
!-------------------------------------------------------------------------
...
! locals
   ...
   type(ESMF_StateItemType)   , pointer  :: typelist(:)
   character(len=ESMF_MAXSTR)            :: name 
...
! start
   ...
   if(itemcount==0) then
      status = 1
      if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' state is empty')
   end if
   ...
   call ESMF_StateGet(STA, itemnamelist=namelist, &
                      stateitemtypelist=typelist, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGet failed')

   do i=1,itemcount
      if(typelist(i)/=ESMF_STATEITEM_FIELD) then
        print *, ' State item is not a field.'
        RETURN_(ESMF_FAILURE)
      end if
   end do
...
! get grid from first field in state, and create bundle
   ...
   call ESMF_StateGetField(STA, trim(namelist(1)), FLD, &
                            rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGetField failed')
   call ESMF_StateGet(STA, name, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGet failed')
   name = name//'to bundle'
   call ESMF_FieldGet (FLD, grid=GRID, rc=status)
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_FieldGet failed')
   ! create bundle with input grid
   BUN = ESMF_BundleCreate ( name=name, &
                             grid=GRID, rc=status )
   if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_BundleCreate failed' )

! populate bundle   
   do i=1,itemcount
     call ESMF_StateGetField(STA, trim(namelist(i)), FLD, &
                             rc=status)
     if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_StateGetField failed')
     if(MAPL_AM_I_ROOT()) print *, ' Got ',trim(namelist(i)),' from field'
     call ESMF_BundleAddField (BUN, FLD, rc=status)
     if (status /= ESMF_SUCCESS) call ESMFL_FailedRC(mype,Iam//' ESMF_BundleAddField failed')
   end do
   RETURN_(ESMF_SUCCESS)
...
! extract fields from bundle and add to a state
   type(ESMF_Bundle), intent(in  ) :: BUN
   type(ESMF_State) , intent(inout):: STA
  integer, intent(out), optional   :: rc
! locals
...
!
   call ESMF_BundleGet (BUN, name=name, FieldCount=NumVars, RC=STATUS)
   if(MAPL_AM_I_ROOT()) print *,' Unwrapping ',trim(name)
   do L=1,NumVars
     call ESMF_BundleGetField (BUN, L, FLD, RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_StateAddField (STA, FLD, rc=status)
   end do
   RETURN_(ESMF_SUCCESS)
...
!-------------------------------------------------------------------
   type(ESMF_Bundle), intent(inout) :: BUN1
   type(ESMF_Bundle), intent(inout) :: BUN2
   type(ESMF_Bundle), intent(inout) :: mergedBUN
   integer, intent(out), optional :: rc
!
   type(ESMF_VM)           :: vm
   type(ESMF_Field)        :: FLD
...
! begin
   ...
   ! get number of fields in bundles
   call ESMF_BundleGet (BUN1, name=name1, FieldCount=NumVars1, RC=STATUS)
   VERIFY_(STATUS)
   call ESMF_BundleGet (BUN2, name=name2, FieldCount=NumVars2, RC=STATUS)
   VERIFY_(STATUS)
   ! mergedBUN is an empty bundle created by the parent routine
   call ESMF_BundleGet (mergedBUN, name=name3,  RC=STATUS)
   VERIFY_(STATUS)
   ...
   ' into ', trim(name3)
   do L=1,NumVars1
     call ESMF_BundleGetField (BUN1, L, FLD, RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_BundleAddField (mergedBUN, FLD, rc=status)
     VERIFY_(STATUS)
   end do ! L
   if(MAPL_AM_I_ROOT()) &
   ...
   ' into ', trim(name3)
   do L=1,NumVars2
     call ESMF_BundleGetField (BUN2, L, FLD, RC=STATUS)
     VERIFY_(STATUS)
     call ESMF_BundleAddField (mergedBUN, FLD, rc=status)
     VERIFY_(STATUS)
   end do ! L
...
! Local variables
     ...
     call ESMF_GridGetDELayout(grid, delayout=layout, rc=status)
     VERIFY_(STATUS)

!    Layout sizes
!    ------------
     call ESMF_DELayoutGet(LAYOUT, localDe=MYID, deCountPerDim = COUNTS, RC=STATUS)
     VERIFY_(STATUS)
     NX = COUNTS(1)
     NY = COUNTS(2)
...
!    -------------------------
     ...
     call ESMF_DELayoutGetDELocalInfo(LAYOUT, de=MYID, coord=COUNTS, rc=status)
     VERIFY_(STATUS)
